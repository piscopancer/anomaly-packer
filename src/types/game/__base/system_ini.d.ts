...
declare global {
  class system_ini {
    ...
    section_exist(section: Section.All): boolean
    line_exist(section: Section.All, line: string): boolean
    line_count(section: Section.All): number
    r_clsid(section: Section.All, line: string): number
    r_bool(section: Section.All, line: string): boolean
    r_token(section: Section.All, line: string, token_list: token_list): number
    /** `r_string` without trimming trailing whitespace */
    r_string_wq(section: Section.All, line: string): string
    r_string(section: Section.All, line: string): string
    r_u32(section: Section.All, line: string): number
    r_s32(section: Section.All, line: string): number
    r_float(section: Section.All, line: string): number
    r_vector(section: Section.All, line: string): vector
    /** @returns `(exists, name, value)` for the line at `line_index` within `section` */
    r_line(section: Section.All, line_index: number): LuaMultiReturn<[exists: boolean, name: string, value: string]>
    close(): void

    // Engine writers & mutators (INI_FILE_EXTENDED_EXPORTS)
    w_bool(section: Section.All, line: string, value: boolean, comment?: string): void
    /** @param value packed `u32` color */
    w_color(section: Section.All, line: string, value: number, comment?: string): void
    w_fcolor(section: Section.All, line: string, value: fcolor, comment?: string): void
    w_float(section: Section.All, line: string, value: number, comment?: string): void
    w_fvector2(section: Section.All, line: string, value: vector2, comment?: string): void
    w_fvector3(section: Section.All, line: string, value: vector, comment?: string): void
    w_fvector4(section: Section.All, line: string, value: vector4, comment?: string): void
    w_s8(section: Section.All, line: string, value: number, comment?: string): void
    w_s16(section: Section.All, line: string, value: number, comment?: string): void
    w_s32(section: Section.All, line: string, value: number, comment?: string): void
    w_s64(section: Section.All, line: string, value: number, comment?: string): void
    w_u8(section: Section.All, line: string, value: number, comment?: string): void
    w_u16(section: Section.All, line: string, value: number, comment?: string): void
    w_u32(section: Section.All, line: string, value: number, comment?: string): void
    w_u64(section: Section.All, line: string, value: number, comment?: string): void
    w_string(section: Section.All, line: string, value: string, comment?: string): void
    /** @returns whether the save succeeded */
    save_as(new_fname?: string): boolean
    save_at_end(b: boolean): void
    remove_line(section: Section.All, line: string): void
    ...
    section_for_each(cb: (this: void, section: Section.All) => boolean): void

    // Nil-safe wrappers and parsers added in Lua (_g.script), not the engine
    r_string_ex(section: Section.All, prop: string): Section.All | null
    r_bool_ex(section: Section.All, prop: string): boolean | null
    r_float_ex(section: Section.All, prop: string): number | null
    /** Parses the value with `xr_logic.parse_condlist` into a condlist (array of condition entries). */
    r_string_to_condlist(section: Section.All, key: string, def_val?: string): any[] | null
    r_list(section: Section.All, key: string, def_val?: string): string[] | null
    /** Wraps `r_line`, returning `(exists, id, value)`. */
    r_line_ex(section: Section.All, key_or_index: string | number): LuaMultiReturn<[exists: boolean, id: string, value: string]>
    /** Splits the value with `parse_names` and returns each name; falls back to `args` when unset. */
    r_mult(section: Section.All, key: string, ...args: any[]): LuaMultiReturn<any[]>
  }
   ...
   *
   * `NonNullable` matters for optional fields. A schema marks a field optional when only some sections of the file carry that line (in an addon's icon-property ltx, `invert` appears in 3 sections of 68), and an optional `invert?: boolean` makes `Schema[S][K]` evaluate to `boolean | undefined`, which is not assignable to `boolean` — so without the unwrap the field resolves to `never` and becomes unreadable by the very reader meant for it. The `-?` above only strips the modifier from this mapped type's own keys, not from the indexed access. Reading an absent line is well-defined at runtime (`r_*_ex` returns nil, hence their `| null` results), so an optional field must stay readable.
   */
  type IniLineKeysOfType<Schema, S extends keyof Schema, T> = {
    [K in keyof Schema[S]]-?: NonNullable<Schema[S][K]> extends T ? K : never
  }[keyof Schema[S]] &
    string
