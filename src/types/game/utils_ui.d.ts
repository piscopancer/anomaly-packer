...
declare global {
  namespace utils_ui {
    ...
    export function get_stats_func_value(obj: CGameObject, sec: Section.Item, file: string, func: string, ...args: unknown[]): number | undefined
    export function get_stats_string_value(obj: CGameObject, sec: Section.Item, gr: string | StatDescriptor, stat: string, to_text: boolean): LuaMultiReturn<[string | number, boolean]>
    export function get_stats_table(sec: Section.Item): Record<string, StatDescriptor> | undefined
    export function get_stats_value(obj: CGameObject, sec: Section.Item, gr: string | StatDescriptor, stat: string): number | false
    export function get_stats_xml(handler: CUIWindow, obj: CGameObject, sec: Section.Item, gr: string | StatDescriptor, stat: string): CUIStatic | undefined
    ...
    export function prop_accuracry(obj: CGameObject, sec: Section.Item): number
    export function prop_condition(obj: CGameObject, sec: Section.Item): number
    export function prop_damage(obj: CGameObject, sec: Section.Item): number
    export function prop_handling(obj: CGameObject, sec: Section.Item): number
    export function prop_rpm(obj: CGameObject, sec: Section.Item): number
  ...
  class UICellContainer {
    ...
    GetCell_SEC(sec: Section.Item): UICellItem | false
    ...
    GetID(obj: CGameObject | undefined, sec: Section.Item, create?: boolean): number | undefined
    AddItemInCell(obj: CGameObject | undefined, sec: Section.Item, indx: number, area?: { y: number; x: number; w: number; h: number }): boolean
    AddItem(obj?: CGameObject, sec?: Section.Item, info?: unknown): number | undefined
    AddItemManual(obj?: CGameObject, sec?: Section.Item, indx?: number): boolean | undefined
    RemoveItem(obj?: CGameObject, sec?: Section.Item): void
    RemoveItem_byID(id: number): void
    RemoveItemManual(indx: number): void
    TransferItem(cont_to: UICellContainer, obj?: CGameObject, sec?: Section.Item): UICellItem | undefined
    UpdateItem(obj?: CGameObject, sec?: Section.Item): void
    FindFreeCell(obj?: CGameObject, sec?: Section.Item): { y: number; x: number; w: number; h: number } | false
    ...
    FindSimilar(obj?: CGameObject, sec?: Section.Item): UICellItem | false
    Grow(): void
  ...
  class UICellItem {
    /** Running height of the list as rows are added, in UI units. */
    H: number
    /** Running width of the list, grown by `AddItemToList` to fit the widest row. */
    W: number
    ...
    Add_Icon(sec: Section.Item, w: number, h: number): void
    Add_Shadow(sec: Section.Item, w: number, h: number): void
    Add_Layers(xml: CScriptXmlInit, obj: CGameObject, sec: Section.Item, clsid: number): void
    Add_ProgressBar(xml: CScriptXmlInit, obj: CGameObject, sec: Section.Item, clsid: number): void
    Add_Counter(xml: CScriptXmlInit, obj: CGameObject, sec: Section.Item): void
    Add_Upgrade(xml: CScriptXmlInit, obj: CGameObject, sec: Section.Item): void
    Add_Attachements(xml: CScriptXmlInit, obj: CGameObject, sec: Section.Item, clsid: number): void
    ...
    Check_TradeMode(obj: CGameObject, sec: Section.Item): boolean
    GetCost(): void
  ...
  /**
   * The arguments a context menu entry's callback is invoked with: the id of the item the
   * menu was opened on, the container it sits in, and whatever extra values the property's
   * `action` tuple carried (its own method name included — vanilla ignores it).
   */
  type CellPropertyParams = [id: number, bag: string, ...args: unknown[]]

  /** @customConstructor UICellProperties */
  class UICellProperties extends CUIScriptWnd {
    /** Running height of the list as rows are added, in UI units. */
    H: number
    ...
    /** Game time an action was taken, or `false` before any; used to swallow the click. */
    action_moment: number | false
    /** Row text size read from `properties:file_item:fn` in xml. */
    file_item_fn_sz: vector2
    /** Row size read from `properties:file_item:main` in xml. */
    file_item_main_sz: vector2
    /** Background static the frame, list and highlight are attached to. */
    form: CUIStatic
    frame: CUIFrameWindow
    /** Hover strip moved over the row under the cursor by `Update`. */
    highlight: CUIStatic
    list_box: CUIListBox
    /** The window whose method an entry's `func` names — the inventory screen in practice. */
    owner: CUIScriptWnd
    ...
    Update(): void
    Reset(
      pos_override: vector2 | undefined,
      action_list: string[],
      name_list: string[],
      params_list: CellPropertyParams[],
    ): void
    FillList(action_list: string[], name_list: string[], params_list: CellPropertyParams[]): void
  ...
  class UICellProperties_item extends CUIListBoxItem {
    /** The row's text field, i.e. the `GetTextItem()` the constructor caches. */
  ...
  class UIInfoItem {
    ...
    InitControls(): void
    Update(obj?: CGameObject, sec?: Section.Item, flags?: AnyTable): void
    Pass(obj?: CGameObject, sec?: Section.Item): boolean
    GetType(sec: Section.Item): string
  ...
  class UIInfoUpgr {
    ...
    Pass(sec: Section.Item): boolean
    ExtractFunctor(sec: Section.Item, param: string, ...args: unknown[]): unknown
    Sync_Y(parent: CUIWindow | undefined, child: CUIWindow, offset?: number): number
    Sync_H(parent: CUIWindow, child: CUIWindow, offset: number): number
    Reset(): void
    IsShown(): boolean
